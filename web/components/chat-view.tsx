...
import type { Channel, Member, Message, Server, User } from "../lib/types";
import { UserAvatar } from "./user-avatar";

function MessageRow({ message, members, currentUser, onDelete }: { message: Message; members: Member[]; currentUser: User; onDelete: (id: string) => void }) {
    const author = message.sender_id === currentUser.id ? currentUser : members.find(m => m.user.id === message.sender_id)?.user;
    const role = members.find(m => m.user.id === message.sender_id)?.role;

    return <div className="group flex gap-4 px-4 py-1.5 hover:bg-black/10"><UserAvatar user={author ?? { username: "?", avatar_url: null }} size={40} /><div className="min-w-0 flex-1"><p className="flex items-baseline gap-2"><span className="text-[15px] font-semibold" style={{ color: role?.name ? undefined : "var(--foreground)" }}>{author?.username ?? "Inconnu"}</span><span className="text-xs text-muted-foreground">{new Date(message.created_at).toLocaleTimeString("fr-FR", { hour: "2-digit", minute: "2-digit" })}</span>{message.sender_id === currentUser.id && <button onClick={() => onDelete(message.id)} className="ml-2 opacity-0 transition-opacity group-hover:opacity-100" aria-label="Supprimer"><Trash2 className="h-3.5 w-3.5 text-muted-foreground hover:text-destructive" /></button>}</p><p className="whitespace-pre-wrap break-words text-[15px] leading-relaxed text-foreground/90">{message.content}</p></div></div>
}

export function ChatView({ channel, messages, members, currentUser, onSend, onDelete, onToggleMembers, membersShown }: { channel: Channel; messages: Message[]; members: Member[]; currentUser: User; onSend: (content: string) => Promise<void>; onDelete: (id: string) => Promise<void>; onToggleMembers: () => void; membersShown: boolean }) {
